...
import com.websit.pay.utils.MD5Util;
...
//			System.out.println("prepay_id:"+prepay_id);
			...
			response.put("timeStamp", timeStamp + "");// 这边要将返回的时间戳转化成字符串，不然小程序端调用wx.requestPayment方法会报签名错误
			 String signs = "appid=" + WxPayConfig.appid + "&noncestr=" + nonce_str + "&package=Sign=WXPay&partnerid="
	                    + WxPayConfig.mch_id + "&prepayid=" + prepay_id + "&timestamp=" + timeStamp + "&key="
	                    + WxPayConfig.key;
			
			// 再次签名，这个签名用于小程序端调用wx.requesetPayment方法
			
			//response.put("paySign", paySign);
			response.put("paySign", MD5Util.MD5Encode(signs, "utf8").toUpperCase());
			response.put("return_code", return_code);
			response.put("return_msg", return_msg);
			response.put("result_code", result_code);
